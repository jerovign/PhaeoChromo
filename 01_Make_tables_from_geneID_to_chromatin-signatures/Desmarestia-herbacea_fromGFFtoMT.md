---
Master table of Desmarestia herbacea
---

This notebook aims to keep track of how I made the MASTER TABLE that is used for all downstream analyses.
*gff* used: `Desmarestia-herbacea.reference.gt.AGAT.gff` derived from `Desmarestia-herbacea.reference.gt.gff` as there were some issue with nf-core/rnaseq.
...
# Define file path
gff_file <- "./data/gff_files/Desmarestia-herbacea.reference.gt.gff"
# Import GFF data
...
salmon_file <- "./data/my_RNAseq_data/Desmarestia-herbacea/salmon.merged.gene_tpm.tsv" 
...
dict_trad <- read_delim('./data/my_RNAseq_data/Desmarestia-herbacea/salmon_tx2gene.tsv', col_names = c('gene_id','trad_id','trad'), show_col_types = FALSE)
head(dict_trad)
...
peaks <- read.table("./data/peak_K79/Desmarestia_herbacea_female_SxS46_H3K79me2_merged-peaks_annotatePeaks.txt", header = TRUE, sep = '\t')
...
```{r, warning=FALSE}
# Load ChIP-seq peak file
peaks <- read.table("./data/peak_K79/Desmarestia_herbacea_male_SxS47_H3K79me2_merged-peaks_annotatePeaks.txt", header = TRUE, sep = '\t')
peaks_gr <- GRanges(seqnames = peaks$Chr,
                    ranges = IRanges(start = peaks$Start, 
                                     end = peaks$End
                                     ))
...
## Step 1: Generates volcano.data - RNA-seq analysis of Desmarestia herbacea between male and female
...
countsToUse <- read.delim("/home/jeromine/Documents/Scripts_Rstudio/MASTER_TABLES/data/my_RNAseq_data/Desmarestia-herbacea/salmon.merged.gene_counts.tsv", header = T, as.is = T)
colnames(countsToUse)[c(3:8)] <- c("DhF1", "DhF2", "DhF3", "DhM1", "DhM2", "DhM3")
...
colData <- data.frame(genotype = c(rep("Female",3), rep("Male",3)), 
...
ggsave(filename = "plotPCA_RNAseqx3_Desmarestia-herbacea.pdf", 
       plot = plotPCA,
       #  units = "cm", 
       # width=10, height=10, 
       path=paste0(output_dir,"output"))

### perform the comparisons
## import TPM table
TPM <- read.delim("./data/my_RNAseq_data/Desmarestia-herbacea/salmon.merged.gene_tpm.tsv", header = T)
colnames(TPM)[1] <- "ID"
# TPM <- TPM[,c(1,2,3,4,6,5,7,8)]
colnames(TPM)[c(3:8)] <- c("DhF1", "DhF2", "DhF3", "DhM1", "DhM2", "DhM3")
TPM <- TPM[,-2]
...
ggsave(filename = "volcanoplot_Desmarestia-herbacea.pdf", 
       plot = last_plot(),
       #  units = "cm", 
       # width=10, height=10, 
       path=paste0(output_dir,"output"))
...
pdf(file ="./output/02_volcano_plots/plotheatmap_Desmarestia-herbacea.pdf", width = 6, height = 6)
pheatmap(matrix, 
         cluster_rows = T,
...
master_table <- master_table %>%
  mutate(
    location = case_when(
      seq_id == 'female_sdr' ~ 'Female-SDR',
      seq_id == 'chr_03' & gene_start >= 5427118 
      & gene_end <= 11995121 ~ 'Male-SDR',
      seq_id == 'chr_03' ~ 'PAR',
      TRUE ~ 'autosome'
    )
  )
...
output_bed <- './data/bed_files_for_plotProfile_quantiles/Desmarestia-herbacea/'
...
hiHMM_female <- read.table("./hiHMM/hihmm.model2.K27.Desmarestia-herbacea-female-SxS46.recoloured.ReMapped.bed", header = FALSE, sep = '\t')

# Load Male hiHMM file
hiHMM_male <- read.table("./hiHMM/hihmm.model2.K27.Desmarestia-herbacea-male-SxS47.recoloured.ReMapped.bed", header = FALSE, sep = '\t')

# Modify seq_id as underscore is not tolerated by hiHMM
hiHMM_female$V1 <- gsub('chr', 'chr_', hiHMM_female$V1)
hiHMM_female$V1 <- gsub('femalesdr', 'female_sdr', hiHMM_female$V1)
hiHMM_female$V1 <- gsub('DherMcontig', 'Dher_M_contig_', hiHMM_female$V1)
hiHMM_male$V1 <- gsub('chr', 'chr_', hiHMM_male$V1)
hiHMM_male$V1 <- gsub('femalesdr', 'female_sdr', hiHMM_male$V1)
hiHMM_male$V1 <- gsub('DherMcontig', 'Dher_M_contig_', hiHMM_male$V1)
...
melt_emissions_into_signature <- function(mt, sex) {
  ...
  ## Handle Missing Columns ---------------------------------------------------
    ...
    if (length(missing_cols) > 0) {
      message(paste("Missing columns:", paste(missing_cols, collapse = ", "), "in Desmarestia h", sex))
      ct[missing_cols] <- 0
    }
  ...
  write.table(states.matrix, "./output/merge_CS/hiHMM_signatures_05032025_Desmarestia_.txt", 
...
```{r}
# Import data
states.matrix <- read.delim("./output/merge_CS/hiHMM_signatures_05032025_Desmarestia_.txt", header = TRUE, sep = "\t")
...
# Open the PDF device
pdf("./output/merge_CS/SignaturesMatrix_05032025_Desmarestia_.pdf", height = 20, width = 3)
...
write.table(master_table, file = './output/07_FINAL_MASTER_TABLES_w-Signature/fmts_Desmarestia-herbacea.csv', 
