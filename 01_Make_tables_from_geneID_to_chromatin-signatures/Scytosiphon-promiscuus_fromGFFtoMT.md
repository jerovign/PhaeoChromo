---
MASTER table for Scytosiphon promiscuus
---

This notebook aims to keep track of how I made the MASTER TABLE that is used for all downstream analyses.
*gff* used: `Scytosiphon-promiscuus_MALE_chr.gff`or `../../gtf-gff-bed-files/Scytosiphon-promiscuus_MALE_chr_2_AGAT.gff`
...
# Define file path
gff_file <- "./data/gff_files/Scytosiphon-promiscuus_MALE_chr.gff"
# Import GFF data
...
salmon_file <- "./data/my_RNAseq_data/Scytosiphon-promiscuus/salmon.merged.gene_tpm.tsv" 
...
salmon_data <- salmon_data %>% select(trad_id, TPM.Female, TPM.Male, log2_TPM_plus_1.Female, log2_TPM_plus_1.Male)
head(salmon_data)

## Step 4: Translate trad_id to gene_id thanks to tx2gene from salmon -----------------------
dict_trad <- read_delim('./data/my_RNAseq_data/Scytosiphon-promiscuus/salmon_tx2gene.tsv', col_names = c('gene_id','trad_id','trad'), show_col_types = FALSE)
head(dict_trad)

## Step 5: Merge TPM values with the master table based on trad_id -----------
salmon_data <- left_join(salmon_data, dict_trad, by = "trad_id") %>% 
  select(-trad)
master_table <- left_join(master_table, salmon_data, by = "gene_id")
print(master_table %>% select(seq_id, gene_id, TPM.Female, TPM.Male, log2_TPM_plus_1.Female, log2_TPM_plus_1.Male))
## Verify the merge was successful and there are no NA values where there
## shouldn't be ------
...
peaks <- read.table("./data/peak_K79/Scytosiphon_promiscuus_female_mr5f_H3K79me2_merged-peaks_annotatePeaks.txt", header = TRUE, sep = '\t')
...
```{r, warning=FALSE}
# Load ChIP-seq peak file
peaks <- read.table("./data/peak_K79/Scytosiphon_promiscuus_male_Asm6_H3K79me2_merged-peaks_annotatePeaks.txt", header = TRUE, sep = '\t')
peaks_gr <- GRanges(seqnames = peaks$Chr,
                    ranges = IRanges(start = peaks$Start, 
                                     end = peaks$End
                                     ))
...
## Step 1: Generates volcano.data - RNA-seq analysis of Scytosiphon promiscuus between male and female
...
countsToUse <- read.delim("./data/Scytosiphon-promiscuus/salmon.merged.gene_counts.tsv", header = T, as.is = T)
countsToUse <- countsToUse[,c(1,2,3,4,6,5,7,8)]
colnames(countsToUse)[c(3:8)] <- c("SpM1", "SpM2", "SpM3", "SpF1", "SpF2", "SpF3")
...
colData <- data.frame(genotype = c(rep("Male",3), rep("Female",3)), 
...
ggsave(filename = "plotPCA_RNAseqx3_Scytosiphon-promiscuus.pdf", 
       plot = plotPCA,
      #  units = "cm", 
      # width=10, height=10, 
      path="./output")
...
## import TPM table
TPM <- read.delim("./data/Scytosiphon-promiscuus/salmon.merged.gene_tpm.tsv", header = T)
colnames(TPM)[1] <- "ID"
TPM <- TPM[,c(1,2,3,4,6,5,7,8)]
colnames(TPM)[c(3:8)] <- c("SpM1", "SpM2", "SpM3", "SpF1", "SpF2", "SpF3")
TPM <- TPM[,-2]

TPM$Male <- rowMeans(TPM[,2:4])
TPM$Female <- rowMeans(TPM[,5:7])
...
ggsave(filename = "volcanoplot_Scytosiphon-promiscuus.pdf", 
       plot = last_plot(),
       #  units = "cm", 
       # width=10, height=10, 
       path="./output/02_volcano_plots/")
...
matrix <- subset(TPM, ID %in% updown$ID)[,c(2:7)]
rownames(matrix) <- updown$ID

matrix <- t(scale(t(matrix), center = T, scale = T))
colours <- rev(brewer.pal(n = 9, name = "RdGy"))

pdf("./output/02_volcano_plots/plotheatmap_Scytosiphon-promiscuus.pdf", width = 6, height = 6)
pheatmap(matrix, 
         cluster_rows = T,
...
names(volcano.data)[1] <- 'trad_id'
volcano.data <- volcano.data %>%  select(trad_id, bias) %>% 
  mutate(bias = ifelse(bias == 'Not significant', NA, bias))
master_table <- left_join(master_table, volcano.data, by = "trad_id")
head(master_table %>%  select(trad_id, TPM.Female, TPM.Male, bias))
```
...
master_table <- master_table %>%
  mutate(
    location = case_when(
      seq_id == 'chr_13' & gene_start >= 4795133 
      & gene_end <= 6159060 ~ 'Male-SDR',
      seq_id == 'chr_13' ~ 'PAR',
      TRUE ~ 'autosome'
    )
  )
  ...
  write_bed <- function(df, quantile_name) {
    ...
    if (nrow(df_filtered) > 0) {
      ...
      fwrite(df_filtered, file = paste0(output_dir, "/", quantile_name, 
                                        "_genes_", sex, ".Scytosiphon-promiscuus.bed"),
             sep = "\t", col.names = FALSE)
    }
  }
...
# output_bed <- './data/05_OverlapEnrichment/Scytosiphon-promiscuus/'
output_bed <- './data/bed_files_for_plotProfile_quantiles/Scytosiphon-promiscuus'
...
hiHMM_female <- read.table("./hiHMM/hihmm.model2.K27.Scytosiphon-promiscuus-female-mr5f.recoloured.ReMapped.bed", header = FALSE, sep = '\t')

# Load Male hiHMM file
hiHMM_male <- read.table("./hiHMM/hihmm.model2.K27.Scytosiphon-promiscuus-male-Asm6.recoloured.ReMapped.bed", header = FALSE, sep = '\t')

# Modify seq_id as underscore is not tolerated by hiHMM
hiHMM_female$V1 <- gsub('chr', 'chr_', hiHMM_female$V1)
hiHMM_female$V1 <- gsub('Sprocontig', 'Spro_contig_', hiHMM_female$V1)
hiHMM_male$V1 <- gsub('chr', 'chr_', hiHMM_male$V1)
hiHMM_male$V1 <- gsub('Sprocontig', 'Spro_contig_', hiHMM_male$V1)
...
overlaps_female <- findOverlaps(mRNA_gr, hiHMM_gr_female, minoverlap = 10)
overlaps_male <- findOverlaps(mRNA_gr, hiHMM_gr_male, minoverlap = 10)
...
melt_emissions_into_signature <- function(mt, sex) {
  ...
  write.table(states.matrix, "./output/merge_CS/hiHMM_signatures_05032025_Scytosiphon_.txt", 
...
```{r}
# # Import data
# states.matrix <- read.delim("./output/merge_CS/hiHMM_signatures_26022025_Scytosiphon_.txt", header = TRUE, sep = "\t")
# 
...
# 
# # Open the PDF device
# pdf("./output/merge_CS/SignaturesMatrix_26022025_Scytosiphon_.pdf", height = 20, width = 3)
# 
# # Create heatmap
...
write.table(master_table, file = './output/07_FINAL_MASTER_TABLES_w-Signature/fmts_Scytosiphon-promiscuus.csv', 
